...
import { NggAccordionModule } from '@sebgroup/green-angular'
...
Simple paragraph content in each section. All sections can be expanded at once since the *closeOthers* input property is set to false.

<Canvas>
    <Story id="components-accordion--simple" />
</Canvas>
...
<Canvas>
    <Story id="components-accordion--nosubheaders" />
</Canvas>


### Checkbox content

Each section contains a set of options with checkboxes, just to illustrate that any content can be contained in each Accordion List Item. Also, expanding one section closes the others since *closeOthers* input property is set to true.

<Canvas>
  <Story id="components-accordion--closeothers" />
</Canvas>
