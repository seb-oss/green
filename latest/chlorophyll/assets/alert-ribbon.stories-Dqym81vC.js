const o={title:"Components/Alert ribbon",tags:["autodocs"],parameters:{componentIds:["component-alertribbon"]},argTypes:{variant:{control:"select",options:["success","warning","danger",""]},heading:{control:"text"},content:{control:"text"},link:{control:"text"},button:{control:"text"},primaryButton:{control:"text"},closeButton:{control:"boolean"},isCloseable:{control:"boolean"},closeText:{control:"text"}}},n={render:t=>`
  <div role="alert" class="gds-alert-ribbon ${t.variant}">
    ...
    ${t.closeButton?'<button class="gds-close" type="button"><span class="sr-only">Close</span><i></i></button>':""}
    ${t.button||t.primaryButton?`<footer>
        ${t.button?`<button type="button" class="gds-button ${["success","danger"].includes(t.variant)?"ghost-light":"ghost-dark"}">${t.button}</button>`:""}
        ${t.primaryButton?`<button type="button" class="gds-button ${["success","danger"].includes(t.variant)?"ghost-light":"ghost-dark"}">${t.primaryButton}</button>`:""}
     </footer>`:""}
  </div>
`,name:"Alert ribbon",args:{variant:"",heading:"Can contain a heading.",content:"Body text starts on the same row as heading. A link (optional) always ends the message, stand alone or",link:"part of the sentence.",button:!1,primaryButton:"",closeButton:!1,isCloseable:!0,closeText:"Close this"}};n.parameters={...n.parameters,docs:{...n.parameters?.docs,source:{originalSource:`{
  render: args => \`
  <div role="alert" class="gds-alert-ribbon \${args.variant}">
    ...
    \${args.closeButton ? \`<button class="gds-close" type="button"><span class="sr-only">Close</span><i></i></button>\` : ''}
    \${args.button || args.primaryButton ? \`<footer>
        ...
        \${args.primaryButton ? \`<button type="button" class="gds-button \${['success', 'danger'].includes(args.variant) ? 'ghost-light' : 'ghost-dark'}">\${args.primaryButton}</button>\` : ''}
     </footer>\` : ''}
  </div>
\`,
  name: 'Alert ribbon',
...
}`,...n.parameters?.docs?.source},description:{story:`[Source code](https://github.com/seb-oss/green/tree/main/libs/chlorophyll/scss/components/alert-ribbon)
&nbsp;|&nbsp;
[Usage guidelines](https://designlibrary.sebgroup.com/components/component-alertribbon)

An alert ribbon is a message used to inform the user about the state of a system, page or function. Icons and color indicate the type and urgency of the information within the message. Note that 'closeButton' will be depricated and replaced by 'isCloseable'.`,...n.parameters?.docs?.description}}};const e=["AlertRibbon"];export{n as AlertRibbon,e as __namedExportsOrder,o as default};
