...
import { IconItem, IconPage, StudioPage, TokenPage } from '../data/studio.types'
...
export default function Aside({
  ...
  const getAllItems = () => {
    switch (page.type) {
      case 'asset':
        return (page as IconPage).icons?.flatMap((group) => group.items) || []
      case 'token':
        return (page as TokenPage).tokens?.flatMap((group) => group.items) || []
      default:
        return []
    }
  }
  ...
  const TabContent = () => {
    ...
    const iconItem = currentItem as IconItem
    switch (activeTab) {
      case 'code':
        return (
          <Core.GdsCard
            ...
            <Core.GdsFlex gap="0" flex-direction="column" padding="0 2xs">
              ...
              >
                <Core.GdsCard
                  ...
                  gap="0"
                >
                  <Core.GdsText font="detail-regular-xs">icon</Core.GdsText>
                  <Core.GdsButton
                    rank="tertiary"
                    size="small"
                    onClick={() => {
