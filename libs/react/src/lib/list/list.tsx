...
export const List = ({ listType, tableCaption, tableData, children, ...props }: ListProps) => {
  if (listType === 'ordered') {
    return <ol {...props}>{children?.map((child, index) => <ListItem key={index}>{child}</ListItem>)}</ol>
    ...
    return (
      <figure>
        ...
        <dl>{tableData?.map((data, index) => <ListItem listType={listType} tableRowData={data} key={index} />)}</dl>
      </figure>
    )
  }
}
