...
const MockComponent = (props: MockComponentProps) => (
  <RadioGroup label="Radio group label" labelInformation="Radio group label information" {...props}>
...
describe('RadioButton Group Component', () => {
  ...
  it('Should fire onChangeRadio', () => {
    ...
    expect(container.querySelectorAll<HTMLInputElement>("input[type='radio']")[0]?.checked).toEqual(true)
    fireEvent.click(screen.getByText('Car 2'))
    expect(mockFn).lastCalledWith('Car 2')
    expect(mockFn).toBeCalledWith('Car 2')
    expect(container.querySelectorAll<HTMLInputElement>("input[type='radio']")[1]?.checked).toEqual(true)
  })
  ...
  it('Should render validator: invalid', () => {
    ...
    expect(screen.getByText('invalid message').classList.contains('form-info')).toEqual(true)
  })
  ...
  it('should be able to set value programmatically', async function () {
    const { container, rerender } = render(<MockComponent defaultSelected={'Car 1'} />)

    rerender(<MockComponent defaultSelected={'Car 1'} valueSelected={'Car 2'} />)

    expect(container.querySelectorAll<HTMLInputElement>("input[type='radio']")[0]?.checked).toBe(false)
    expect(container.querySelectorAll<HTMLInputElement>("input[type='radio']")[1]?.checked).toBe(true)

    rerender(<MockComponent defaultSelected={'Car 1'} valueSelected={'Car 1'} />)
