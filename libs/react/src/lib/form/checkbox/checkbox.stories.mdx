import { Meta, Story, Canvas } from '@storybook/addon-docs'
import { Checkbox } from '../input/input'

export const CheckboxTemplate = ({ ...props }) => <Checkbox {...props} />

<Meta title="Components/Checkbox" component={Checkbox} />

# Checkbox

<Canvas>
  <Story
    name="Default"
    parameters={{
      componentIds: ['component-checkbox']
    }}
    args={{ label: 'Checkbox input', defaultChecked: false }}>
    {CheckboxTemplate.bind({})}
  </Story>
</Canvas>

## Checkbox state

You control the state of the checkbox by setting the `checked` prop to true or false.

```tsx
function MyComponent () {
  const [isChecked, setIsChecked] = useState<boolean>()

  return (
    <Checkbox
      label="Example label"
      checked={isChecked}
      onChange={(event) => { setIsChecked(event.target.checked) }
    />
  )
}

```

## Default Checked

If you don't use the `checked` prop and `onChange` handler. You can still control the checked state on initial render with `defaultChecked`. Doing this you probably get the checked value at a later stage, like submitting a form.

<Canvas>
  <Story
    name="DefaultChecked"
    parameters={{
      componentIds: ['component-checkbox']
    }}
    args={{ label: 'Checkbox input', defaultChecked: true}}>
    {CheckboxTemplate.bind({})}
  </Story>
</Canvas>

## States and variants checkbox

<Canvas>
  <form>
    <Checkbox label="Normal unchecked" />
    <Checkbox label="Normal checked" checked />
    <Checkbox label="Invalid unchecked" validator={{
             indicator: 'error',
             message: 'Invalid'
           }} />
    <Checkbox label="Invalid checked" validator={{
             indicator: 'error',
             message: 'Invalid'
           }} checked />
  </form>
</Canvas>

## Validation

<Canvas>
  <Story name="invalid"
         args={{
           label: 'Invalid',
           validator: {
             indicator: 'error',
             message: 'Error'
           }
         }}
  >
    {CheckboxTemplate.bind({})}
  </Story>
</Canvas>

## Props

| Prop              | Type                                                    | Description                                                                       |
| :---------------- | :------------------------------------------------------ | :-------------------------------------------------------------------------------- |
| label             | `string`                                                | The label of the checkbox                                                         |
| checked?          | `boolean`                                               | Use this prop to control the checked state of the checkbox                        |
| defaultChecked?   | `boolean`                                               | Use this if you don't control the checkbox with the `checked` prop but still want the checkbox to be checked on render.                     |
| onChange?         | `(e: React.FormEvent<HTMLInputElement>) => void`        | A handler function that takes the event and can act on it                         |
| validator?        | `IValidator`                                            | An object used to render the validation status for the component                  |
| testId?           | `string`                                                | A test id that will be placed in the `data-testid` attribute on the input element |



