...
const TableBody = React.forwardRef(({ ...props }: TableBodyProps, ref: React.ForwardedRef<HTMLTableSectionElement>) => {
  ...
  const cloneTableRow = React.useCallback((Child: React.ReactElement<any>, index: number) => {
    const isTableRow: boolean =
      React.isValidElement<React.FC<TableRowProps>>(Child) && (Child?.type as any)?.displayName === 'TableRow'
  ...
  return (
    <tbody {...props} ref={ref}>
      {React.Children.map(props.children as React.ReactElement, (Child: React.ReactElement<any>, i: number) => {
        if (Child?.type === React.Fragment) {
          return React.cloneElement<any>(Child, {
            children: React.Children.map(
              Child.props.children,
              (FragmentChild: React.ReactElement<any>, fragmentIndex: number) =>
