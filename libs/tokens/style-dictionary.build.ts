...
const themes = {
  ...
  ios: {
    ...
    source: [__dirname + `/src/tokens/ios/**/*.json`],
  },
}
...
await Promise.all(
  Object.entries(themes).map(([theme, themeConfig]) => {
    colorSchemes.map((colorScheme) => {
      const config: Config = {
        include: [
          __dirname + `/src/tokens/${themeConfig.baseTheme}/**/*.ref.json`,
          __dirname +
            `/src/tokens/${themeConfig.baseTheme}/**/tokens.base.json`,
          __dirname +
            `/src/tokens/${themeConfig.baseTheme}/**/*.typography.json`,
          __dirname +
            `/src/tokens/${themeConfig.baseTheme}/**/*.comfortable.json`,
          __dirname +
            `/src/tokens/${themeConfig.baseTheme}/**/*.${colorScheme}.json`,
        ],
