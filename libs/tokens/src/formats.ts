...
import { Format, Options, TransformedToken, formatHelpers } from 'style-dictionary'
import { color } from '@storybook/theming'

const { fileHeader, sortByReference, createPropertyFormatter, sortByName, setSwiftFileProperties } = formatHelpers

const useColorScheme = (allTokens: TransformedToken[], options: Options): TransformedToken[] => {
...
const formats: Record<string, Format> = {
  'css/theme': {
    ...
    formatter: function (args) {
      ...
      const options: Options = Object.assign({ colorScheme: 'light', selector: ':root' }, args.options)
  ...
  'color/v2': {
    ...
    formatter: function (args) {
      const dictionary = Object.assign({}, args.dictionary)
      const options = Object.assign({ selector: ':host', colorScheme: 'light' }, args.options)
      ...
      dictionary.allTokens = dictionary.allTokens
        ...
        .map(token => {
          if (token.path[2] === 'l1' || token.path[2] === 'l2' || token.path[2] === 'l3') {
      ...
      const cssVariables = dictionary.allTokens
        .map(token => {
          let value = options.colorScheme === 'dark' ? token.darkValue : token.value
      ...
      return formatHelpers.fileHeader({ file: args.file }) + `color-scheme: ${options.colorScheme};\n` + cssVariables
    }
  },
  size: {
    name: 'size',
    formatter: function (args) {
      ...
      dictionary.allTokens = dictionary.allTokens.map(token => {
        if (token.path[0] === 'sys' && token.path[1] === 'space') {
          ...
          const adjustedName = token.name.replace(/(\d)-/, '$1').replace('sys-', '')
          return Object.assign({}, token, {
            name: adjustedName,
  ...
  text: {
    ...
    formatter: function (args) {
      ...
      dictionary.allTokens = dictionary.allTokens.map(token => {
        if (
          (token.path[0] === 'sys' && token.path[1] === 'typography' && token.path[2] === 'size') ||
          (token.path[0] === 'sys' && token.path[1] === 'typography' && token.path[2] === 'line-height')
        ) {
          // Adjust token name to remove hyphen after numbers
          const adjustedName = token.name.replace(/(\d)-/, '$1').replace('sys-', '').replace('typography-', 'text-')
          return Object.assign({}, token, {
            name: adjustedName,
            value: `${token.value}px`,
            original: { value: `${token.value}px` }
          })
        } else if (token.path[0] === 'sys' && token.path[1] === 'typography' && token.path[2] === 'weight') {
          // Only adjust token name for 'weight', without changing the value
          const adjustedName = token.name.replace('sys-', '').replace('typography-', 'text-')
          return Object.assign({}, token, {
            name: adjustedName
    ...
    formatter: function (args) {
      ...
      const combinedShadows = Object.entries(shadowGroups)
        .map(([size, props]) => {
          ...
          const color1 = hexToRgba(props[`${size}-color-1`] || '#000000', opacityMap[size][0])
          ...
          const color2 = hexToRgba(props[`${size}-color-2`] || '#000000', opacityMap[size][1])
      ...
      return formatHelpers.fileHeader({ file: args.file }) + `${options.selector} {\n` + combinedShadows + `\n}\n`
    }
  },
  'figma/json': {
    name: 'json/figma',
  ...
  'scss/mixin': {
    ...
    formatter: function (args) {
      const dictionary = Object.assign({}, args.dictionary)
      const options = Object.assign({ colorScheme: 'light', name: 'add-variables' }, args.options)
  ...
  'green/ios-swift-package': {
    ...
    formatter: function ({ options, file }) {
      const template = _template(fs.readFileSync(process.cwd() + '/libs/tokens/src/templates/ios/spm.package.template'))
  ...
  'green/ios-swift-class': {
    ...
    formatter: function ({ dictionary, options, file, platform }) {
      const template = _template(
        fs.readFileSync(
          process.cwd() + '/node_modules/style-dictionary/lib/common/templates/ios-swift/any.swift.template'
        )
      )
  ...
  'green/android-resources': {
    ...
    formatter: function ({ dictionary, options, file }) {
      const template = _template(
        fs.readFileSync(
          process.cwd() + '/node_modules/style-dictionary/lib/common/templates/android/resources.template'
        )
      )
