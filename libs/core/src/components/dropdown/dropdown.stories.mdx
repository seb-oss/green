...
A common use case for the small version is in constrained areas, such as table cells. In those cases, it is also often desirable to hide the label. This can be done with the `hide-label` attribute.

Omitting the label attribute itself is not recommended, as it would make the dropdown inaccessible to screen readers.
...
<Canvas>
  <div
    style={{
      ...
      dummy: registerTransitionalStyles(),
    }}
  >
