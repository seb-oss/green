...
```html
<gds-grid columns="8 4 2" gap="xxl xs xxxl">
    <!-- Child elements here -->
</gds-grid>
```
...
```html
<gds-grid columns="8 4 2" padding="xxl xs xxxl">
    <!-- Child elements here -->
</gds-grid>
```
...
## Example 
This example shows a grid with 8 columns on desktop, 4 on tablet, and 2 on mobile. It also has a gap of `xxl` on desktop, `m` on tablet, and `xs` on mobile. The padding is `xxl` on desktop, `xs` on tablet, and `xxxl` on mobile.
<Canvas>
...
<Canvas>
    <gds-grid columns="8 4 2" gap="xxl m xs" fluid>
        <div>COL: 01</div>
        <div>COL: 02</div>
        <div>COL: 03</div>
        <div>COL: 04</div>
