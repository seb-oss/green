...
describe('<gds-datepicker>', () => {
  describe('Rendering', () => {
    it('should render a datepicker', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker></gds-datepicker>`)
  ...
  describe('API', () => {
    it('should set the value', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker value="2024-01-01"></gds-datepicker>`)
      await expect(el.value).to.deep.equal(new Date('2024-01-01'))
    })

    it('should set the min', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker min="2024-01-01"></gds-datepicker>`)
      await expect(el.min).to.deep.equal(new Date('2024-01-01'))
    })

    it('should set the max', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker max="2024-01-01"></gds-datepicker>`)
      await expect(el.max).to.deep.equal(new Date('2024-01-01'))
    })

    it('should set open', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker open></gds-datepicker>`)
      const popover = el.shadowRoot!.querySelector<GdsPopover>('#calendar-popover')!
    ...
    it('should set the label', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker label="Date"></gds-datepicker>`)
      await expect(el.label).to.equal('Date')
    })

    it('should set the date format', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker dateformat="d/m/y"></gds-datepicker>`)
      const spinners = el.shadowRoot!.querySelectorAll<GdsDatePartSpinner>(getScopedTagName('gds-date-part-spinner'))!
      const separator = el.shadowRoot!.querySelector<HTMLSpanElement>('.field .input > span')!
    ...
    it('should dispatch a change event when the value is changed by the user', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker value="2024-01-10"></gds-datepicker>`)
    ...
    it('should reset when form is reset', async () => {
      ...
      const resetButton = el.querySelector<HTMLButtonElement>('button[type="reset"]')!
      const spinners = el
        .querySelector('#datepicker')!
        .shadowRoot!.querySelectorAll<GdsDatePartSpinner>(getScopedTagName('gds-date-part-spinner'))!
    ...
    it('should return a focused date', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker value="2024-01-10" open></gds-datepicker>`)
      const focusedDate = await el.getFocusedDate()

      await expect(onlyDate(focusedDate!)).to.equal(onlyDate(new Date('2024-01-10')))
    })
    ...
    it('Setting `disabled-weekends` should disable weekends', async () => {
      const el = await fixture<GdsDatepicker>(
        html`<gds-datepicker value="2024-01-10" disabled-weekends open></gds-datepicker>`
      )
    ...
    it('Setting `disabled-dates` should disable dates', async () => {
      const el = await fixture<GdsDatepicker>(
        html`<gds-datepicker value="2024-01-10" disabled-dates="2024-01-13" open></gds-datepicker>`
      )
  ...
  describe('Interactions', () => {
    ...
    it('should set year to 1900 when blurring the year spinner with a value below 1900', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker min="2022-01-01"></gds-datepicker>`)
      const spinners = el.shadowRoot!.querySelectorAll<GdsDatePartSpinner>(getScopedTagName('gds-date-part-spinner'))!
      spinners[0].focus()
      await sendKeys({
        type: '1'
    ...
    it('should be possible to type out a date without tabbing', async () => {
      ...
      await expect(onlyDate(el.value!)).to.equal(onlyDate(new Date('2024-05-10')))
    })
    ...
    it('should reset to initial value when pressing escape in the popover', async () => {
      const el = await fixture<GdsDatepicker>(html`<gds-datepicker value="2024-01-01"></gds-datepicker>`)
      const button = el.shadowRoot!.querySelector<HTMLButtonElement>('[aria-controls="calendar-popover"]')!
      const popover = el.shadowRoot!.querySelector<GdsPopover>('#calendar-popover')!
      ...
      await expect(onlyDate(el.value!)).to.equal(onlyDate(new Date('2024-01-01')))
    })
  })
})
