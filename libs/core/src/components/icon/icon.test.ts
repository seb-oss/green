...
describe('IconArchive', () => {
  ...
  it('should render the regular SVG when solid is false', async () => {
    ...
    expect(svg?.innerHTML.trim()).to.equal('<path d="M5 3l14 9-14 9V3z"></path>')
  })
  ...
  it('should render the solid SVG when solid is true', async () => {
    ...
    await element.updateComplete
    const svg = element.shadowRoot?.querySelector('svg')
    expect(svg?.innerHTML.trim()).to.equal('<path d="M5 3v18l14-9L5 3z"></path>') // Change this line
  })
  it('should be accessible', async () => {
    element = await fixture(html`<gds-icon-arrow label="Arrow"></gds-icon-arrow>`)
