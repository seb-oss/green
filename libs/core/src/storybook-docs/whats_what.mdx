...
All the components in Green Core is implemented as [Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) using the [Lit](https://lit.dev/) library. This means that they are framework agnostic and can be used with any framework, or even without a framework. The components makes use of [Shadow DOM](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM) to encapsulate their styles and behavior, so they won't interfere with the rest of your application.

The reason the components use Shadow DOM is twofold:

- Provide style encapsulation to protect against conflicts which often occurs in a micro-frontend environment
