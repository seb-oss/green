...
export class TabsComponent implements OnChanges, OnInit, AfterContentInit, OnDestroy {
  /** @internal */
  @HostBinding('class.sdv-tabs') baseClass = true;

  /** Special property used for selecting DOM elements during automated UI testing. */
  @HostBinding('attr.data-thook') @Input() thook: string | null | undefined = 'tabs';

  /** @internal List of children with a {@link TabDirective}. */
  @ContentChildren(TabDirective) tabRefs: QueryList<TabDirective> = new QueryList();
        ...
        .subscribe((e: NavigationEnd) => {
          ...
          if (tab) {
            ...
            if (tab.disabled) {
              ...
              const fallbackTab = this.selectedTab ? this.selectedTab : this.tabs.find(({ disabled }) => !disabled);
              if (fallbackTab && fallbackTab.link) this.router.navigateByUrl(fallbackTab.link);
            }
          }
        }),
    );
  ...
  protected updateTabs() {
    ...
    // Select or give up
    if (selectedTab && this.selectedTab !== selectedTab) this.onSelect(selectedTab.id);
  }
  ...
  onSelect(id: string | number | undefined) {
    ...
    if (selectedTab && !selectedTab.disabled && selectedTab !== this.selectedTab) {
      let cancelled = false;
      ...
      if (cancelled) {
        this.ngvTabSwitch.emit({ type: 'cancel', prev: this.selectedTab, next: selectedTab });
        this.isSwitching = false;
        return;
      }
      if (selectedTab.link) {
        // Navigate and wait for guard accept route
        this.router.navigateByUrl(selectedTab.link, { replaceUrl: true }).then((changed) => {
          if (changed === false) {
            this.ngvTabSwitch.emit({ type: 'cancel', prev: this.selectedTab, next: selectedTab });
            this.isSwitching = false;
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    switch (event.key) {
      case 'End': // End
        if (parent) this.tabFocusHelper(event, parent.lastElementChild as HTMLElement);
        break;

      case 'Home': // Home
        if (parent) this.tabFocusHelper(event, parent.firstElementChild as HTMLElement);
        break;

      case 'LeftArrow': // Left arrow
      case 'UpArrow': // Up arrow
        this.tabFocusHelper(event, tabElement.previousElementSibling as HTMLElement);
        break;
