  ...
  describe('FoldOutComponent - constructor()', () => {
    ...
    describe('setVisibility', () => {
      ...
      it('subscribes to click and keydown events if menu expanded', () => {
        const keyDownSpy = jest.spyOn(component as any, 'subscribeToKeyDownEvent');
        const clickSpy = jest.spyOn(component as any, 'subscribeToOutsideClickEvent');
    ...
    describe('handleArrowUpClick', () => {
      it('sets `activeIndex` value to latest child index if `firstArrowClick` is true', () => {
        ...
        expect((component as any).activeIndex).toEqual((component as any).children.length - 1);
      });
