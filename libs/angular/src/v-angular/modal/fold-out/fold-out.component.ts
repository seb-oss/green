...
export class NgvFoldOutComponent implements OnDestroy, AfterViewInit {
  /** Special property used for selecting DOM elements during automated UI testing. */
  @HostBinding('attr.data-thook') @Input() thook: string | null | undefined = 'fold-out'
  ...
  private blurElement = (element: HTMLElement): void => element.setAttribute(this.ariaFocusAttributeName, 'false');
  private focusElement = (element: HTMLElement): void => element.setAttribute(this.ariaFocusAttributeName, 'true');
  private getChildIndex = (element: HTMLElement | undefined): number => (element ? this.children.indexOf(element) : -1);

  private onClickSubscription: Subscription | undefined;
  private onKeyUpSubscription: Subscription | undefined;

  private get children() {
    return Array.from((this.container?.nativeElement as HTMLElement).children ?? []) as HTMLElement[];
  }
  ...
  private subscribeToKeyDownEvent(): void {
    ...
    this.onKeyUpSubscription = fromEvent<KeyboardEvent>(document, 'keydown')
      .pipe(
        ...
        filter((event) => Object.values(KeyboardKey).includes(event.code as KeyboardKey)),
      )
      .subscribe((event) => {
  ...
  private handleArrowDownClick(): void {
    this.activeIndex = this.firstArrowClick ? 0 : (this.activeIndex + 1) % this.children.length;
    this.firstArrowClick = false;
    this.handleFocus();
  }
