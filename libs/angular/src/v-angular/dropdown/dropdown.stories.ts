...
const CustomSelectedTemplate: StoryFn<StoryArgs> = (args: any) => {
  ...
  return {
    ...
    styles: [
      `
      ::ng-deep.ngg-field-dropdown__label::before {
        margin: 0.5rem 0;
      }
      .account-option {
        margin: 0.5rem 0;
      ...
      <ngg-dropdown
        ...
        <ng-template let-option #optionTpl>
          ...
          <div class="account-option" *ngIf="!!option.key">
            <div class="account-option--label">
              <div>
                <ngg-typeahead-highlight [textContent]="option?.accountName" [textToHighlight]="getFilterPhrase()">
                </ngg-typeahead-highlight>
              </div>
            ...
            <div class="account-option--number">
              <div>
                <ngg-typeahead-highlight [textContent]="option?.accountNumber" [textToHighlight]="getFilterPhrase()">
                </ngg-typeahead-highlight></div>
              <div></div>
            </div>
          </div>
        </ng-template>
