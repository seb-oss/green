...
import { applicationConfig, Meta, moduleMetadata, Story, StoryFn } from '@storybook/angular';
...
export default {
  ...
  decorators: [
    ...
    moduleMetadata({
      ...
      imports: [CommonModule, FontAwesomeModule, FormsModule, ReactiveFormsModule, NgvI18nModule, TypeaheadDirective],
    }),
  ],
} as Meta;
...
const options = [
  ...
  { key: 'opt5', label: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do' },
  {
    label: 'Group',
    options: [
...
type StoryArgs = DropdownComponent<string, string | null, ExtendedOption<string, string | null>> & WithExtras;
...
export const WithCustomSelectedTemplateTypeahead = CustomSelectedTemplateTypeahead.bind({});
        ...
        value$.pipe(
          ...
          map((inputValue) =>
            options.filter((option: any) =>
              option.label?.replace(/\s/gm, '').toLocaleLowerCase().includes(inputValue?.replace(/\s/gm, '').toLocaleLowerCase()),
            ),
          ),
        ),
    },
