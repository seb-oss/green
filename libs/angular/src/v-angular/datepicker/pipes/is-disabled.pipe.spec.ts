...
describe('[NgvDatepicker]', () => {
  ...
  describe('IsDisabledPipe - constructor()', () => {
    ...
    `(`firstValid set to $firstValidStr`, ({ firstValid }: { firstValid: Date }) => {
    ...
    describe.each`
      ...
      ${firstOfJanuary} | ${firstOfJanuary.toISOString().substring(0, 10)}
      ${lastOfDecember} | ${lastOfDecember.toISOString().substring(0, 10)}
    `(`lastValid set to $lastValidStr`, ({ lastValid }: { lastValid: Date }) => {
      let pipe: IsDisabledPipe;
      let config: DisableDateConfig;
    ...
    `(`closingTime set to '$closingTimeStr'`, ({ closingTime }: { closingTime: Date }) => {
      let pipe: IsDisabledPipe;
      let config: DisableDateConfig;
