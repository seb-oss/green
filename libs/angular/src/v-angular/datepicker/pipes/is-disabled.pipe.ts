...
export class IsDisabledPipe implements PipeTransform {
  ...
  transform(value: Date, config: DisableDateConfig = {}): boolean {
    ...
    if (closingTime && match(value, TODAY) && afterClosingHours(closingTime)) return true;

    // if fromDate was provided and the date occurs before fromDate date
    if (fromDate && (!match(value, fromDate) && isBefore(value, fromDate))) return true;
    ...
    if (excludeDays && excludeDays.length && excludeDays.indexOf(value.getDay()) !== -1) return true;
