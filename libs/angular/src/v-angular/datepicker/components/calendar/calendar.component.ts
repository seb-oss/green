...
import { generateDateMatrix, getDayOffset, sortWeekDays } from '../../datepicker.utils';
...
export class CalendarComponent implements OnChanges {
  ...
  @ViewChildren(CalendarDateDirective) dateButtonRefs!: QueryList<CalendarDateDirective>;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.firstDayOfWeek?.currentValue) {
      this.lastDayOfWeek = sortWeekDays(changes.firstDayOfWeek.currentValue).pop() as WeekDay;
    }

    if (
      (changes.month?.currentValue !== undefined && changes.month?.currentValue !== null) ||
  ...
  keyNavigation(event: KeyboardEvent): void {
    ...
    if (event.key !== 'Tab') activeElement = document.activeElement as HTMLElement;
  ...
  getElementWithOffset(fromElement: HTMLElement | undefined, offsetDays: number, returnOnlyEnabled: boolean = true): HTMLElement | undefined {
    if (!fromElement) return;
    const fromIndex = this.dateButtonRefs.toArray().map(ref => ref.nativeElement).indexOf(fromElement);
    const next = this.dateButtonRefs.get(fromIndex + offsetDays);
    if (!next) return;
    if (returnOnlyEnabled && next.disabled) return this.getElementWithOffset(next.nativeElement, offsetDays);
  ...
  firstDayOfWeekElement(): HTMLElement | undefined {
    ...
    const dateRef = this.dateButtonRefs.find(ref => ref.nativeElement === activeElement) as CalendarDateDirective;
    ...
    const targetElement = this.getElementWithOffset(activeElement, offset, false) as HTMLButtonElement;
    if (targetElement && targetElement.disabled) return this.getNextFocusable(targetElement);
  ...
  lastDayOfWeekElement(): HTMLElement | undefined {
    ...
    const targetElement = this.getElementWithOffset(activeElement, offset, false) as HTMLButtonElement;
    if (targetElement && targetElement.disabled) return this.getPreviousFocusable(targetElement);
    return targetElement;
  }
