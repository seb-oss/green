  ...
  describe('DatepickerComponent', () => {
    ...
    beforeEach(
      waitForAsync(() => {
        TestBed.configureTestingModule({
          ...
          imports: [CommonModule, NgvDatepickerTestingModule],
          providers: [],
        }).compileComponents();
      }),
    );
    ...
    describe('getDisabledDatesFor()', () => {
      ...
      const generateDisabledDates = (): Date[] => {
        ...
        let lastGeneratedDate = new Date(`${today.getFullYear() - 1}-${today.getMonth() + 1}-1`);
      ...
      `(
        ...
        ({ targetDate, previous, next }: { targetDate: Date; previous: number; next: number }) => {
          const prevDt = new Date(targetDate);
          prevDt.setDate(-1);
          const nextDt = new Date(targetDate);
          nextDt.setDate(33);
          it(
            `returns disabled dates for ${dateFormat(prevDt)}, ${dateFormat(targetDate)} and ${dateFormat(nextDt)}`,
            () => {
            const disabledDateInterval = component.getDisabledDatesFor(new CalendarMonth(targetDate), disableDatesArr);
            // expect to return dates only for these three months
            disabledDateInterval.forEach((disableDate) => {
              expect([previous, targetDate.getMonth(), next].includes(disableDate.getMonth()));
            });
            const intervalForMonth = (month: number) =>
              disabledDateInterval.filter((disableDate) => disableDate.getMonth() === month);
