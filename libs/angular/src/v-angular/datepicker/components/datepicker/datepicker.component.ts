...
export class DatepickerComponent implements OnInit, OnChanges, OnDestroy {
  @ViewChild('calendarTemplate') calendarTemplate: TemplateRef<any> | null = null;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.selected && !!changes.selected.currentValue && !changes.selected.isFirstChange()) {
  ...
  ngOnInit(): void {
    ...
    // initiate calendar
    const initDate = !!this.selected && isValid(this.selected) ? new Date(this.selected) : new Date();
  ...
  getDisabledDatesFor(calendar: CalendarMonth, disableDates?: (string | Date)[]): Date[] {
    ...
    return disableDates
      ...
      .filter(
        (d: Date) =>
          withinSameMonth(d, previousMonth) || withinSameMonth(d, targetMonth) || withinSameMonth(d, nextMonth),
      );
  }
  ...
  dateClickHandler(date: Date): void {
    ...
    if (date.getMonth() === this.activeCalendar.month) return this.ngvDateChange.emit(date);
