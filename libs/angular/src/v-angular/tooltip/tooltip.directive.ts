...
export class TooltipDirective implements AfterViewInit, OnChanges, OnDestroy {
  ...
  constructor(private anchorElementRef: ElementRef, private renderer: Renderer2) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      TooltipDirective.shouldUpdate(changes.ngvTooltip) ||
  ...
  hide(destroy = false) {
    ...
    if (this.parentElement.contains(this.tooltipElement)) this.renderer.removeChild(this.parentElement, this.tooltipElement);
  ...
  create(text: string) {
    ...
    this.renderer.addClass(this.arrowElement, `sdv-tooltip__arrow-${this.placement}`);
