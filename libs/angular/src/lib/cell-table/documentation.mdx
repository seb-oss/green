...
When using `clickableRows`, you must also make sure to add the `clickLabel` input to provide a label for the chevron.

The full row is clickable with a mouse or touch input, but when navigating using keyboard or screen reader, the chevron
will be the only focusable element that can be used to trigger the click action.
...
| clickLabel      | `string`                     | The label for the chevron.                      |
