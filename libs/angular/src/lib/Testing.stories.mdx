...
Because some of the components in Green are Web Components, and Jest runs in a Node environment, Jest needs some extra configuration to work with Green.

For example, if you run into errors like `Jest encountered an unexpected token`, this is probably the reason.

To make this is seamless as possible, we provide some pre-packaged configuration that you can add.

First, you need to make sure your jest config is in Javascript and not Typescript. If you have a `jest.config.ts` file, rename it to `jest.config.cjs`.

The, change the `const config = { ... }` to `module.exports = { ... }`. Also remove the `export default config` line.

Then, you can extend your configuration like this:

```js
// import green jest config
const greenJestConfig = require('@sebgroup/green-core/green-jest-config.cjs')
...
To interact with Green web components in a test, you can import types from the `@sebgroup/green-core` package, nd use that when querying the DOM.

```ts
it('should render disply correct value in dropdown', async () => {
