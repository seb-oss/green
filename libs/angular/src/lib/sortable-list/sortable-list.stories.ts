...
import { SortableListItem, NggSortableListComponent } from './sortable-list.component'
...
const createTemplate = (suffixTemplate = false): StoryFn<NggSortableListComponent> => {
  return args => ({
    template: `
...
const onItemSelected = (changedItem: SortableListItem, items: SortableListItem[]) => {
  console.log('Updated item:', changedItem, items)
}

const onItemOrderChanged = (event: { previousIndex: number; currentIndex: number; items: SortableListItem[] }) => {
  ...
  groups: [
    {
      ...
      items: [
        ...
        },
        {
          ...
          selected: false,
          description: '"Surely deleting this unused variable won\'t cause any issues in production."',
