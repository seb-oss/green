...
export const bbanPattern: RegExp = /(\d{4})(\d{2})(\d{3})/;
export const ibanPattern: RegExp = /[a-zA-Z0-9]{1,4}/g;
 ...
 */
export const formatAccountNumber = (value: string, schema?: AccountNumberSchema): string => {
  // evalue value schema to 'iban' or 'bban'
  if (!schema) schema = evaluateValueSchema(value);

  // if schema 'iban', return formatted value 
  if (schema === SchemaType.BBAN) return value.replace(bbanPattern, '$1\u00A0$2\u00A0$3\u00A0');

  // if schema 'iban', remove all white-spaces and other separators from the value
  const trimmedValue = value.replace(/\s|-/g, '');
  if (schema === SchemaType.IBAN) return trimmedValue.replace(ibanPattern, (match: string, offset: number) => offset ? `\u00A0${match}` : match)
...
export class AccountNumberPipe implements PipeTransform {
  transform = (value: string, schema?: AccountNumberSchema): string => formatAccountNumber(value, schema);
}
