...
> implements OnInit, OnDestroy
{
  ...
  private createInput() {
    ...
    this.inputComponent = this.viewContainerRef.createComponent(TypeaheadInputComponent);
  ...
  private createDropdownList() {
    this.dropdownListComponent = this.viewContainerRef.createComponent(TypeaheadDropdownListComponent);
  ...
  private setOptions(filteredValues: T[], input: string) {
    ...
    let allowNullish =
      this.allowUnselect &&
      !input &&
      !(Object.keys(filteredValues[0]).includes('key') && filteredValues[0].key == null);
