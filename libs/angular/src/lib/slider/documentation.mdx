import { Canvas, Story } from '@storybook/addon-docs'

import { NggSliderComponent } from './slider.component'

# Slider

A slider to experiment and adjust values with.

## Description

The component allows for easy adjustments of a value and check the updated result immediately.

## Usage

On pages where the user can experiment or adjust values in a scale, such as "How much can I borrow?".

## Importing module

If you're only using the Slider component, you just need to import the `NggSliderModule`.

```typescript
import { NgModule } from '@angular/core'
import { BrowserModule } from '@angular/platform-browser'
import { NggSliderModule } from '@sebgroup/green-angular'

import { AppComponent } from './app.component'

@NgModule({
  declarations: [AppComponent],
  imports: [BrowserModule, NggSliderModule],
  exports: [],
})
export class AppModule {}
```

## Slider Options

### Inputs

| Input        | Type      | Description                                                                           |
| :----------- | :-------- | :------------------------------------------------------------------------------------ |
| name         | `string`  | Name of the input element                                                             |
| min          | `number`  | The greatest value in the range of permitted values. Default is 0                     |
| max          | `number`  | The lowest value in the range of permitted values. Default is 100                     |
| step         | `number`  | The number that specifies the granularity that the value must adhere to. Default is 1 |
| label        | `string`  | Label for the input element                                                           |
| disabled     | `boolean` | Whether or not the input is disabled                                                  |
| hasTextbox   | `boolean` | Whether or not should have input textbox along with the slider                        |
| errorMessage | `string`  | Error message for invalid inputs                                                      |

### Outputs

| Event        | Description                                                            |
| :----------- | :--------------------------------------------------------------------- |
| sliderChange | Event emitted with the slider value when slider value has been changed |
| sliderTouch  | Event emitted with `true` when slider has been focused                 |

## Examples

### Simple

<Canvas>
  <Story id="components-slider--simple" />
</Canvas>

### With textbox

<Canvas>
  <Story id="components-slider--textbox" />
</Canvas>

### With error message

<Canvas>
  <Story id="components-slider--error" />
</Canvas>

### Disabled

<Canvas>
  <Story id="components-slider--disabled" />
</Canvas>
