...
import { DialogComponent, DialogEvent } from '@sebgroup/ngv-modal';
...
@Component({
  selector: 'ngv-drag-drop',
  templateUrl: './drag-drop.component.html',
  styleUrls: ['./drag-drop.component.scss'],
})
export class DragDropComponent implements OnInit, OnDestroy {
  /** @internal */
  @ViewChild('deleteModalRef') deleteModalRef!: DialogComponent;

  /** Special property used for selecting DOM elements during automated UI testing. */
  @HostBinding('attr.data-thook') @Input() thook = 'drag-drop';
    ...
    if (this.list) {
      this.service.fetchFiles(this.fetchStatuses).subscribe((apiFiles) => {
        for (const apiFile of apiFiles) {
          ...
          if (extendedFile.uploadState === 'uploaded' || extendedFile.uploadState === 'validating') {
  ...
  onBeforeUnload(event: Event) {
    if (!this.allUploadingDone(this.files)) {
      ...
      ((event as any) || (window.event as any)).returnValue = confirmationMessage;
  ...
  onDrop(event: DragEvent | Event): void {
    ...
    } else if (event.target && event.target instanceof HTMLInputElement && event.target.files) {
  ...
  onRemove(id: string, state: UploadState): void {
    ...
    if (file.uploadState === 'error' || file.uploadState === 'aborted' || file.uploadState === 'deleted') {
    ...
    this.files.forEach((file) => {
      ...
      file.uploadRequests = this.service!.uploadFile(file.raw!).subscribe(
        (event) => {
          switch (event.type) {
            ...
            case HttpEventType.Response:
              ...
              // Check if validation is required, then start polling
              if (file.uploadState === 'uploaded' || file.uploadState === 'validating') {
        ...
        (error) => {
          ...
          file.status = 'aborted';
          // todo: wrong
          file.statusReasonInformation = [{ originatorId: 'local', reason: 'dragDrop.error.MFEFHM001' }];
          this.stateChange.emit(pick(file, 'uploadState', 'fileName', 'id', 'status', 'statusReasonInformation'));
          this.loading = false;
        },
      );
  ...
  private checkValidity(file: LocalFile & APIFile, delay: number) {
    ...
    file.uploadRequests = timer(0, delay)
      .pipe(
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        switchMap(() => this.service!.fetchStatus(file.id)),
        retryWhen(
          retryStrategy({
  ...
  private fallbackError(code: string | null) {
