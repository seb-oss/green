...
|:--------|:----------|:------|
|locale|Locale used with datepicker. | `navigator.language` |
|currentDate|Initial date used by datepicker. | `new Date()` |
|selectedDate|Selected date.|n/a|
|closeOnSelect|Should datepicker close on select.| `true` |
|useCurrentTime| Use current time when selecting date.| `true` |
|showWeeks|Show week numbers.| `false` |
|weekName|Header for week numbers.| `{ abbr: 'Week', displayText: 'wk' }` |
