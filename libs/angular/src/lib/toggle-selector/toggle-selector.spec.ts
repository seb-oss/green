...
describe('ToggleSelectorComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      declarations: [NggToggleSelectorComponent, ToggleSelectorTestComponent],
    }).compileComponents()
  ...
  it('Testing two-way data binding', async () => {
    ...
    const ngModelChange = spyOn(
      component.toggleSelector,
      'writeValue'
    ).and.callThrough()
    const generateDisplayArray = spyOn(
      component.toggleSelector,
  ...
  it('Should disable one element when disabled prop is set to true', () => {
    const inputEl: DebugElement[] = fixture.debugElement.queryAll(
      By.css('input')
    )
    expect(
      inputEl.some((el) => el.nativeElement.disabled === true)
    ).toBeTruthy()
    expect(
      inputEl.every((el) => el.nativeElement.disabled === true)
    ).toBeFalsy()
  })
  ...
  describe('Things that should happen when multi is true', () => {
    ...
    it('Should have type checkbox when multi is true', () => {
      const elements: DebugElement[] = fixture.debugElement.queryAll(
        By.css('.toggle-selector input[type=checkbox]')
      )
      expect(elements.length).toBe(component.list.length)
    })
    ...
    it('Should have two checked when both options are clicked', () => {
      const element: HTMLElement[] =
        fixture.nativeElement.querySelectorAll('.toggle-selector')
      ...
      expect(
        inputEl.every((el) => el.nativeElement.checked === true)
      ).toBeFalsy()
  ...
  describe('Things that should happen when multi is false', () => {
    ...
    it('Should have type radio when multi is false', () => {
      const elements: DebugElement[] = fixture.debugElement.queryAll(
        By.css('.toggle-selector input[type=radio]')
      )
      expect(elements.length).toBe(component.list.length)
    })
