...
const _format = (date: Date, options: Intl.DateTimeFormatOptions, locale: string) => (
...
export const months = ({
  ...
  const year = Array(12).fill(null).map(
    (_, month) => (
      _format(new Date(`1970-${_paddedString(month + 1)}-01`), { month: format }, locale)
    )
  )
