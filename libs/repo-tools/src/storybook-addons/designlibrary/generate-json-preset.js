...
module.exports = {
  ...
  webpackFinal: async (config, options) => {
    ...
    const storySpecifiers = coreCommon.normalizeStories(await options.presets.apply('stories'), directories)
    const storyPaths = {}

    await Promise.all(
      storySpecifiers.map(async s => {
        const fullGlob = slash(path.join(directories.workingDir, s.directory, s.files))
    ...
    try {
      const jsonOutput = await Promise.all(
        Object.keys(storyPaths).map(async absPath => {
          const csf = (await csfTools.readCsfOrMdx(absPath, { makeTitle: s => s })).parse()
      ...
      const jsonOutputFlat = jsonOutput.reduce((cur, acc) => [...acc, ...(Array.isArray(cur) ? [...cur] : [cur])], [])
...
const getStoryParametersFromCSF = (csf, storyName) => {
  ...
  Object.keys(storyAnnotations).forEach(key => {
    if (storyAnnotations[key].storyName?.value === storyName) storyParams = storyAnnotations[key].parameters?.properties
  })
