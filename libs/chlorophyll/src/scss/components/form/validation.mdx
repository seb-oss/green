...
Using `.is-valid` and/or `.is-invalid`, form inputs can either be marked as valid, invalid or neutral depending on which class that has been applied to the input. If no class has been applied but the `.gds-form-info` element is present, it's considered to be neutral i.e. just plain info related to the input.
