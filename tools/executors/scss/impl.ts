...
export default async function multipleExecutor(options: MultipleExecutorOptions, context: ExecutorContext) {
  ...
  const result = await Promise.race([
    await runExecutor({ project: context.projectName, target: 'compile-scss' }, {}, context),
    // copy readme and changelog
    await new Promise(resolve =>
      copyfiles([`libs/${context.projectName}/*.md`, options.outputPath], { up: 2 }, resolve)
    )
    ...
    // copy scss files
    await new Promise(resolve =>
      copyfiles([`libs/${context.projectName}/scss/**/*.scss`, `${options.outputPath}`], { up: 2 }, resolve)
    )
      .catch(_ => [{ success: false }])
      .then(_ => [{ success: true }])
  ])
