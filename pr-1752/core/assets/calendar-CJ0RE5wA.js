        ...
        ${ue(this.focusedDate,e=>m`
            ${e.map(a=>m`
                <tr role="row">
                  ...
                  ${a.days.map(r=>{const o=this.customizedDates&&this.customizedDates.find(v=>w(v.date,r)),s={color:"currentColor",disabled:!!(this.disabledDates&&this.disabledDates.some(v=>w(v,r))),...o},i=!oe(this.focusedDate,r),d=(r<this.min||r>this.max)&&!w(r,this.min)&&!w(r,this.max),l=r.getDay()===0||r.getDay()===6,n=s.disabled||i||d||this.disabledWeekends&&l;return this.hideExtraneousDays&&i?m`<td inert></td>`:m`
                          <td
                            role="${T(n?void 0:"gridcell")}"
                            class="${j({"custom-date":!!o,disabled:!!n,today:w(t,r)})}"
                            ?disabled=${n}
                            tabindex="${w(this.focusedDate,r)?0:-1}"
                            aria-selected="${this.value&&w(this.value,r)?"true":"false"}"
